...
const productCreateValidator = [
 body("sku")
 .exists({ checkFalsy:false}).withMessage("SKU is required").isLength({min:3, max:50}).withMessage("SKU min 8 max 50"),
 body("name")
 .exists({ checkFalsy:false}).withMessage("Name is required").isLength({min:3, max:50}).withMessage("Name min 8 max 50"),
 body("brand_id")
